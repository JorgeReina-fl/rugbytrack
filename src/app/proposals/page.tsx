...
export default async function ProposalsRouterPage({ searchParams }: PageProps) {
      ...
      <div className="min-h-screen bg-background text-foreground pb-10">
        ...
        <main className="mx-auto max-w-3xl px-4 py-16 text-center">
          ...
          <p className="mt-2 text-xs font-mono uppercase tracking-widest font-bold text-muted-foreground">
            Debes unirte a un equipo o crear uno para poder ver las propuestas.
          </p>
  ...
  if (teams.length === 1) {
    redirect(`/teams/${teams[0]!.id}/proposals`);
  }

  if (!teamIdParam) {
    redirect(`/proposals?teamId=${teams[0]!.id}`);
  }
    ...
    <div className="min-h-screen bg-background text-foreground pb-10">
      ...
      <main className="mx-auto max-w-4xl px-4 py-10 space-y-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <h1 className="text-5xl font-heading font-extrabold uppercase tracking-tighter">Propuestas</h1>
            <p className="text-muted-foreground font-mono uppercase text-xs tracking-widest font-semibold mt-2">
              Actividades propuestas en {selectedTeam.name}
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Link
              href={`/teams/${selectedTeam.id}/proposals`}
